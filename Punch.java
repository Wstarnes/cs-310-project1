...
public class Punch {
    ...
    public void adjust(Shift s) {
        ...
        if(shiftStart.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY || shiftStart.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY){
            ...
            //check-in punches
            if (punchtypeid == 1) {
                if(originalTimeStampMillis <= startGraceMillis && originalTimeStampMillis >= startIntervalMillis){
                    adjustedTimeStamp.setTimeInMillis(shiftStartMillis);
                    eventdata = "Shift Start";
                }
                else if(originalTimeStampMillis <= startIntervalMillis){
                    adjustedTimeStamp.setTimeInMillis(shiftStartMillis);
                    eventdata = "Interval Round";
                }
            ...
            //check-out punches
                ...
                if(originalTimeStampMillis >= stopGraceMillis && originalTimeStampMillis <= stopIntervalMillis){
                    adjustedTimeStamp.setTimeInMillis(shiftStopMillis);
                    eventdata = "Shift Stop";
                }
                else if(originalTimeStampMillis >= stopIntervalMillis){
                    adjustedTimeStamp.setTimeInMillis(stopIntervalMillis);
                    eventdata = "Interval Round";
                }
                //start of lunch
                else if(originalTimeStampMillis >= lunchStartMillis && originalTimeStampMillis < lunchStopMillis){
